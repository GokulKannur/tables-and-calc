  ...
  return (
    <div className="max-w-7xl mx-auto flex gap-8">
      ...
      <aside className="hidden md:block w-64">
        <div className="p-4 bg-white border rounded-lg shadow-sm sticky top-24">
          <h2 className="text-lg font-semibold mb-4 text-slate-800">All Converters</h2>
          <nav>
            <ul className="space-y-2">
              {converterCategories.map((cat) => (
                <li key={cat.name}>
                  <Link 
                    href={cat.href}
                    className="block p-2 rounded-md text-slate-600 hover:bg-slate-100 hover:text-blue-600 transition-colors"
                  >
                    {cat.name}
                  </Link>
                </li>
