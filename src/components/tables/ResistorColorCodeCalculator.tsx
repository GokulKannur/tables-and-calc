...
export default function ResistorColorCodeCalculator() {
  ...
  useEffect(() => {
    const rawValue = (band1 * 10 + band2) * multiplier;
    let displayValue;
    if (rawValue >= 1000000) {
  ...
  const findColor = (bandList: any[], value: any) => bandList.find(b => b.value === value)?.color || 'White';
    ...
    <div className="bg-white p-6 border rounded-lg shadow-sm">
      ...
      <div className="flex justify-center items-center my-8">
        ...
        <div className="w-48 h-20 bg-amber-200 rounded-lg flex items-center justify-center gap-3 px-4 border-2 border-amber-300">
          ...
          <div className={`w-4 h-20 ${colorMap[findColor(bandsData.multiplier, multiplier)]} border-x`}></div>
